...
%                   otherwise.
    ...
    s = featureMatrix;
    x = patches;
    cost = (1.0) * sum(sum((A * s - x) .* (A * s - x))) + lambda * sum(sum(sqrt((groupMatrix * (s .* s) + epsilon))));
    cost = cost + gamma * sum(sum(A .* A));
    grad = (2.0) * A' * (A * s - x) + lambda * groupMatrix' * (1.0 ./ sqrt(epsilon + groupMatrix * (s .* s))) .* s;
    grad = grad(:);
end
